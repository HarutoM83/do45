...
public:
    ...
    void loop() noexcept 
    {
        while (windowInstance_.messageloop()) {
            ...
            commandQueueInstance_.getcq()->ExecuteCommandLists(_countof(ppCommandLists), ppCommandLists);

            // プレゼント
            swapChainInstance_.getsc()->Present(1, 0);

            // フェンスにフェンス値を設定
            commandQueueInstance_.getcq()->Signal(fenceInstance_.get(), nextFenceValue_);
